...
static void compute_l1_diag(const dcsr_mat* A, REAL** out) {
  INT n = A->row;
...
void amg_build_hierarchy(amg_data* amg, const dcsr_mat* A) {
  ...
  lev->cf_order = NULL;
  lev->l1_diag = NULL;
  compute_l1_diag(&lev->A, &lev->l1_diag);

  INT coarsening = amg->coarsening;
  const char *coarsen_name = (coarsening == COARSEN_PMIS) ? "PMIS" :
                             (coarsening == COARSEN_HMIS) ? "HMIS" : "RS";

  fprintf(stderr,
    "Building %s hierarchy (threshold=%.2f, min_size=%d)\n",
    coarsen_name, threshold, min_size);
  ...
  while (Ac->row >= min_size && nlev < max_levels) {
    ...
    INT n_coarse = 0;
    if (coarsening == COARSEN_PMIS)
      pmis_coarsening(&cur->A_filtered, cf, &n_coarse);
    else if (coarsening == COARSEN_HMIS)
      hmis_coarsening(&cur->A_filtered, cf, &n_coarse);
    else
    ...
    next->cf_order = NULL;
    next->l1_diag = NULL;
    compute_l1_diag(&next->A, &next->l1_diag);
  ...
  fprintf(stderr, "%s hierarchy complete: %d levels (coarsest ichol nnz=%d)\n\n",
          coarsen_name, nlev, coarsest->L_ichol.nnz);
}
...
void amg_rebuild_values(amg_data* amg, const dcsr_mat* A_new) {
  ...
  if (lev0->l1_diag) { free(lev0->l1_diag); lev0->l1_diag = NULL; }
  compute_l1_diag(&lev0->A, &lev0->l1_diag);
  ...
  for (INT k = 0; k < nlev - 1; k++) {
    ...
    next->A = Ac_new;
    if (next->l1_diag) { free(next->l1_diag); next->l1_diag = NULL; }
    compute_l1_diag(&next->A, &next->l1_diag);
  }
...
void amg_free(amg_data* amg) {
  for (INT k = 0; k < amg->num_levels; k++) {
    ...
    if (lev->l1_diag) { free(lev->l1_diag); lev->l1_diag = NULL; }
