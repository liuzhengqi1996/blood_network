 ...
 *   Build AMG from Ap (matrixP.csr), ichol from A (matrixA.csr), solve Ax=b.
 *
...
int main(int argc, char** argv) {
  ...
  INT  min_size  = (argc > arg_offset + 2) ? atoi(argv[arg_offset + 2]) : 128;
  ...
  /* Incomplete Cholesky of solve matrix A */
  if (two_matrix)
    fprintf(stderr, "Computing ichol of solve matrix %s ...\n", solve_file);
  ...
  if (two_matrix)
    fprintf(stderr, "\nPCG solve of %s with AMG(%s)+ichol(%s):\n",
            solve_file, amg_file, solve_file);
  else
